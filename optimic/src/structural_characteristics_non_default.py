...
def junction_length(tessellation_og, log_level):
    ...
    *** specifically for 2D case. Just an example to show modularity of code ***
    ...
    Returns
    -------
    An array of junction lengths.
    """
    # Creating deep copy of original tessellation dictionary
    tessellation = copy.deepcopy(tessellation_og)

    ## Computing junction lengths
    ScipyRidgeDictList = tessellation['ScipyRidgeDictList']
    vorVert = tessellation['ScipyVoroVertices']
    ridgeLenArr=[]
    for akey,aval in ScipyRidgeDictList:
        if any(val<tessellation['number_of_grains'] for val in akey):
            ridgelen=distance.euclidean(vorVert[aval[0]],vorVert[aval[1]])
            ridgeLenArr.append([0, 0, ridgelen])
    
    return np.array(ridgeLenArr)

def for_angle_junction_length(tessellation_og, log_level):
    """
    Compute junction lengths.
...
def junction_angle(tessellation_og, log_level):
    ...
    grains_with_junction, edges_of_junction = for_angle_junction_length(tessellation, log_level)
...
def distance_btw_grains(dimension, limit, tessellation_og, log_level):
    ...
    *** specifically for 2D case. Just an example to show modularity of code ***
    ...
    """
    ## Creating deepcopy of original tessellations
    tessellation = copy.deepcopy(tessellation_og)

    ScipyRidgeDictList = tessellation['ScipyRidgeDictList']
    vorVert = tessellation['ScipyVoroVertices']
    vorPts = tessellation['ScipyPoints']

    ## Computing distance between neighbors
    neighDistArr=[]
    for akey,aval in ScipyRidgeDictList:
        if any(val<tessellation['number_of_grains'] for val in akey):
            dist=distance.euclidean(vorPts[akey[0]],vorPts[akey[1]])
            neighDistArr.append(dist)
    
    ## Returning only 1D array in this case
    return np.array(neighDistArr), np.array(neighDistArr)
