...
def cubic_3d_testcase(tessellation, dimension, size_of_simulation_box, \
    ...
    """
    Execute all assert statements related to cubic_3d_testcase.
    ...
    try:
        ...
        assert np.all([np.isclose(num[2], 6) for num in number_of_neighbor])   # The no. of neighbors should be the same                
