...
Module to test some features of program for HCP 2D type spacing.
...
def hcp_2d_testcase(tessellation, dimension, size_of_simulation_box, \
    ...
    """
    Execute all assert statements related to hcp_2d_testcase.
    ...
    try:
        ...
        for v in range(copy.deepcopy(tessellation['number_of_grains'])):
            assert np.isclose(copy.deepcopy(tessellation['number_of_faces_list'][v]) - 2, 6)                       ## ignoring surfaces with surface normals along z axis. REMEMBER !!!!!!
        
        ## Grain size distribution
        assert np.all([grain_size_distributions[:, 1] == grain_size_distributions[0, 1]])     # The grain sizes should be the same
        assert np.allclose(grain_size_distributions[:, 1], (1.0501)*spacing_length, atol=1e-4)
        ...
        assert np.all([np.isclose(num[2], 6) for num in number_of_neighbor])
        
        ## GB area
        assert np.all([np.isclose(area[3], grain_boundary_area_distribution[0][3]) for area in grain_boundary_area_distribution])
        assert np.all([np.isclose(area[3], 0.57735*spacing_length*length_z, atol=1e-2) for area in grain_boundary_area_distribution])
    ...
    log.info('hcp_2d_testcase passed !!')