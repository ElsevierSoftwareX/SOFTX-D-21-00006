...
Module to test some features of program for HCP 3D type spacing.
...
def hcp_3d_testcase(tessellation, dimension, size_of_simulation_box, \
    ...
    """
    Execute all assert statements related to hcp_3d_testcase.
    ...
    try:
        ...
        assert np.all([np.isclose(num[1], 10) for num in number_of_neighbor])                           # The no. of neighbors should be the same
    except AssertionError:
        log.exception('hcp_3d_testcase failed !!')

    log.info('hcp_3d_testcase passed !!')